...
//        mList.addAll(carResponse.getOrderData());
        ...
        //设置点击事件
        storeAdapter.addChildClickViewIds(R.id.iv_checked_store);
...
//
    ...
    private void isSelectAllStore(boolean state) {
        //修改背景
        ivCheckedAll.setImageDrawable(getDrawable(state ? R.drawable.ic_checked : R.drawable.ic_check_diy));
    ...
    public void checkedStore(int shopId,boolean state) {
        ...
        if(shopIdList.size() == mList.size()){
            ...
            //不全选
            ivCheckedAll.setImageDrawable(getDrawable(R.drawable.ic_check_diy));
            isAllChecked = false;
        }
        ...
        for (int i = 0; i < mList.size(); i++) {
            ...
            for (int j = 0; j < orderDataBean.getCartlist().size(); j++) {
                ...
                if (cartlistBean.isChecked()) {
                    ...
                    Log.d("cart",cartlistBean.getProductName());
                }
            }
        }
